  ...
  return (
    <div className="ui-page relative isolate overflow-hidden pb-12 pt-2 md:pb-16">
      ...
      <header className="sticky top-4 z-40">
        <div className="premium-frame rounded-2xl px-4 py-3 md:px-5 md:py-3.5">
          <div className="flex items-center justify-between gap-3">
            ...
            <Link
              ...
              className="ui-btn ui-btn-primary premium-nav-active shrink-0 rounded-xl px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.08em]"
            >
              JUEGA YA
            </Link>
          </div>
            ...
            <div className="flex min-w-max items-center gap-2">
              {NAV_LINKS.map((item) => (
                <a
                  key={item.href}
                  href={item.href}
                  className="ui-btn ui-btn-secondary rounded-full px-3 py-1.5 text-[11px] font-medium tracking-[0.08em] text-zinc-200 transition hover:border-white/35"
                >
                  {item.label}
                </a>
              ))}
      ...
      <section id="inicio" className="mt-8 grid gap-4 lg:grid-cols-[1.35fr_0.65fr]">
        <article className="premium-hero relative overflow-hidden rounded-3xl p-6 md:p-8">
          <div
            aria-hidden="true"
            className="pointer-events-none absolute -right-8 -top-8 h-40 w-40 rounded-full bg-[#f2c94c]/12 blur-3xl"
          />
          <p className="premium-kicker text-xs">Fantasy Liga 1 Peru 2026</p>
          <h1 className="premium-title mt-3 max-w-3xl text-3xl font-semibold leading-tight text-white md:text-5xl">
        ...
        <aside className="grid gap-3">
          <div className="premium-block rounded-3xl border p-5">
            <p className="premium-kicker text-xs">Por qué entrar</p>
      ...
      <section id="como-jugar" className="mt-10 space-y-4">
        <div className="flex items-end justify-between gap-3">
          <h2 className="premium-title text-2xl font-semibold text-white">Cómo se juega</h2>
          <p className="premium-kicker text-[11px]">4 pasos</p>
        </div>
        <div className="grid gap-3 md:grid-cols-2">
          <div className="premium-block rounded-2xl border p-4">
      ...
      <section id="ranking" className="mt-10 space-y-3">
        <h2 className="premium-title text-2xl font-semibold text-white">Ranking Top 10</h2>
        {leaderboardLoading ? (
          <div className="premium-block rounded-2xl border p-4 text-xs text-zinc-400">
            Cargando Top 10...
          </div>
        ) : null}
        {leaderboardError ? (
        ...
        {!leaderboardLoading && !leaderboardError ? (
          <div className="premium-block rounded-2xl border p-3">
              ...
              {leaderboard.length === 0 ? (
                ...
                leaderboard.map((entry) => (
                  <div
                    ...
                    className="grid grid-cols-[56px_1fr_auto] items-center gap-2 rounded-xl border border-white/12 bg-black/25 px-2 py-2 text-xs"
                  >
      ...
      <section id="premium" className="mt-10 space-y-3">
        <h2 className="premium-title text-2xl font-semibold text-white">Premium</h2>
        {premiumError ? (
          <div className="rounded-2xl border border-warning/40 bg-warning/10 p-4 text-xs text-warning">
            {premiumError}
          </div>
        ...
        {!premiumError && premiumConfig ? (
          <div className="space-y-3">
            <div className="grid gap-3 md:grid-cols-3">
              {premiumPlans.map((plan) => {
                return (
                  <div
                    key={plan}
                    className="premium-block rounded-2xl border p-4 shadow-[0_15px_35px_rgba(0,0,0,0.25)]"
                  >
                    ...
                    <button
                      type="button"
                      disabled
                      className="mt-4 w-full cursor-not-allowed rounded-xl border border-white/12 bg-black/25 px-3 py-2 text-xs font-semibold uppercase tracking-[0.08em] text-zinc-400"
                    >
                      Próximamente
                    </button>
                  </div>
            ...
            {!premiumConfig.can_buy_apertura ? (
              <div className="premium-block rounded-2xl border p-3 text-xs text-zinc-400">
      ...
      <section id="fixtures" className="mt-10 space-y-3">
        <h2 className="premium-title text-2xl font-semibold text-white">Próximos partidos</h2>
        {roundsError ? (
          <div className="rounded-2xl border border-warning/40 bg-warning/10 p-4 text-xs text-warning">
            {roundsError}
          </div>
              ...
              >
                {[block.slice(0, 5), block.slice(5, 10)].map((column, columnIndex) => (
                  <div
                    ...
                    className="premium-block rounded-2xl border p-3"
                  >
                    <div className="space-y-2">
                      {column.map((fixture) => {
                        ...
                        return (
                          <div
                            key={fixture.id}
                            className="rounded-xl border border-white/12 bg-black/25 px-3 py-2 text-xs text-zinc-400"
                          >
      ...
      <section id="faq" className="mt-10 space-y-3">
        <h2 className="premium-title text-2xl font-semibold text-white">FAQ</h2>
        <div className="grid gap-3 md:grid-cols-2">
          <div className="premium-block rounded-2xl border p-4 text-sm text-zinc-300">
          ...
          <div className="premium-block rounded-2xl border p-4 text-sm text-zinc-300 md:col-span-2">
