...
if (-not (Test-Path $envFile)) {
  Write-Error "Missing .env.prod at $envFile"
  exit 1
}
...
Write-Host "Starting services (prod)..."
if ($NoBrowser) {
  & $startAll -Env prod -SkipWatch -NoBrowser
} else {
  & $startAll -Env prod -SkipWatch
}
