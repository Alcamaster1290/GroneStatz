...
if (-not (Test-Path $envFile)) {
  Write-Error "Missing .env.qa at $envFile"
  exit 1
}
...
Write-Host "Starting services (qa)..."
if ($NoBrowser) {
  & $startAll -Env qa -SkipWatch -NoBrowser
} else {
  & $startAll -Env qa -SkipWatch
}
